...
func (a *Applier) applyObject(ctx context.Context, object *unstructured.Unstructured) error {
	...
	if err != nil {
		a.fireError(ApplierObjectError, object, err)
		return err
	}
...
func (a *Applier) waitDeleted(ctx context.Context, object *unstructured.Unstructured) error {
	// Retrieve the metadata of the object to check if it is already deleted:
	gvk := object.GetObjectKind().GroupVersionKind()
	metadata := &metav1.PartialObjectMetadata{}
	metadata.SetGroupVersionKind(gvk)
	key := clnt.ObjectKeyFromObject(object)
	err := a.client.Get(ctx, key, metadata)
	if apierrors.IsNotFound(err) {
		return nil
	}
	if err != nil {
	...
	// If it is not deleted then we need to watch till we receive the deleted event:
	list := &metav1.PartialObjectMetadataList{}
	list.SetGroupVersionKind(gvk)
	watch, err := a.client.Watch(
		ctx,
		list,
		clnt.InNamespace(metadata.Namespace),
		clnt.MatchingFields{
	...
	if !deleted {
		...
		if metadata.Namespace != "" {
			name = fmt.Sprintf("%s/%s", metadata.Namespace, name)
		}
		return fmt.Errorf(
			"timed out while waiting for object '%s' of kind '%s' to be completely "+
				"deleted",
			name, metadata.Kind,
		)
	}
