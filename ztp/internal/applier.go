...
// AddLabel adds a label that will be added to all the objects created.
...
// AddLabels adds a the collection of labels that will be added to all the objects created.
...
func (a *Applier) applyObject(ctx context.Context, object *unstructured.Unstructured) error {
	...
	object, err := a.copyObject(object)
	if err != nil {
		return err
	}

	// Create a logger with some additional details about the object:
	...
	logger.V(2).Info("Created object")
	...
	if status != nil {
		...
		logger.V(2).Info("Updated status of object")
	}
