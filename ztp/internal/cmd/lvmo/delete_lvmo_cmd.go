...
// Delete creates and returns the `delete lvmo` command.
func Delete() *cobra.Command {
	c := NewDeleteCommand()
	result := &cobra.Command{
		Use:     "lvmo",
		Aliases: []string{"lvmos"},
		Short:   "Deletes logical volumen manager operator",
		Args:    cobra.NoArgs,
		RunE:    c.run,
	}
	flags := result.Flags()
	...
	_ = flags.Bool(
		...
		"Enabled or disables deletion of the custom resource definitions of the "+
			"'lvm.topolvm.io' group that are created when the logical volumne manger "+
			"operator is created. This is itended for use in testing and development "+
			"environments and shouldn't be used in production environments.",
	)
	return result
}

// DeleteCommand contains the data and logic needed to run the `delete lvmo` command.
...
// NewDeleteCommand creates a new runner that knows how to execute the `delete lvmo` command.
func NewDeleteCommand() *DeleteCommand {
	return &DeleteCommand{}
}

// run runs the `delete lvmo` command.
...
func (t *DeleteTask) deleteLVMO(ctx context.Context) error {
	// Create the applier:
	...
	if crds {
		t.console.Warn("CRDs in group 'lvm.topolvm.io' will be deleted")
		err = t.deleteCRDGroup(ctx, "lvm.topolvm.io")
		if err != nil {
			return err
		}
	}
