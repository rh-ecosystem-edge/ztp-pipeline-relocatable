...
// Setup creates and returns the `dev setup` command.
...
// SetupCommand contains the data and logic needed to run the `dev setup` command.
type SetupCommand struct {
}

// NewSetupCommand creates a new runner that knows how to execute the `dev setup` command.
func NewSetupCommand() *SetupCommand {
	return &SetupCommand{}
}

// run executes the `dev setup` command.
func (c *SetupCommand) run(cmd *cobra.Command, argv []string) (err error) {
	// Get the context:
	ctx := cmd.Context()
