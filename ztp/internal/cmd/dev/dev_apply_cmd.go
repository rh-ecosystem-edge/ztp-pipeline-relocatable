...
// Apply creates and returns the `dev apply` command.
...
// ApplyCommand contains the data and logic needed to run the `dev apply` command.
...
// NewApplyCommand creates a new runner that knows how to execute the `dev apply` command.
func NewApplyCommand() *ApplyCommand {
	return &ApplyCommand{}
}

// run executes the `dev apply` command.
func (c *ApplyCommand) run(cmd *cobra.Command, argv []string) error {
	// Get the context:
	ctx := cmd.Context()
