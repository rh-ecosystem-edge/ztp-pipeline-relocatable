xref:config[config]:
  ...
  xref:OC_OCP_VERSION[OC_OCP_VERSION]: "4.9"
  xref:OC_OCP_TAG[OC_OCP_TAG]: "4.9.0-x86_64"
  xref:OC_RHCOS_RELEASE[OC_RHCOS_RELEASE]: "49.84.202110081407-0" # TODO automate it to get it automated using binary
...
xref:spokes[spokes]:
  - xref:spokename[spoke1-name]:
      xref:mastername[master0]:
        xref:nic_ext_dhcp[nic_ext_dhcp]: eno4 # ext_dhcp -> DHCP
        xref:nic_int_static[nic_int_static]: eno5 # int_static -> Internal IP for cluster communication
