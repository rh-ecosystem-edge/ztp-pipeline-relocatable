...
This script will deploy OpenShift-Pipelines and enable the Tasks and Pipelines in the Hub cluster under the `spoke-deployer` Namespace.
...
| This `spokes.yaml` file will contain the configuration for all the clusters you want to deploy at the same time. You need to put it with a `cat` command as we do in the example execution.
| Yes
...
include::modules/ztp-for-factory-pipeline-hub.adoc[leveloffset=+1]
include::modules/ztp-for-factory-pipeline-edge.adoc[leveloffset=+1]
