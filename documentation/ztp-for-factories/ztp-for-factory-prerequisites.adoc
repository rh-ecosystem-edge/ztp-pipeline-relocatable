...
- We need some Existant PVs on the HUB
    . We cannot use `emptyDir` directive because between each step in the pipeline, the contents will be removed and we require them to further progress.
    ...
    . We need to fill the Openshift Storage requirements for the Hub like (SSD/NVME).
    . LSO should be enough but we recommend to use a more reliable one like ODF or NFS in order to avoid issues with the PVs and node scheduling pods.
...
|[[root_disk]] `root_disk`
| Disk device to be used for OS installation
