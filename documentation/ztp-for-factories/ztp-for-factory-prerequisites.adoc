...
- PVC's defined for the HUB
    . It must not be an `emptydir` because between each step in the pipeline, the contents will be removed and we require them to further progress.
- DNS entries configured and resolvable from both internal and external network, with DNS on the DHCP Factory network
    - HUB
        . `api.hub.<domain>` and `api-int.hub.<domain>` entries to the same IP address
        . ingress (*.apps.hub.<domain>)
    - SPOKE
        . `api.<spokename>.<domain>` and `api-int.<spokename>.<domain>` entries to the same IP address
        . ingress (*.apps.<spokename>.<domain>)
