...
function install_odf_sno() {
    ...
    echo "Filling vars for ${edgecluster}"
    extract_vars ".edgeclusters[].${edgecluster}.master0.storage_disk"
    oc --kubeconfig=${EDGE_KUBECONFIG} apply -f manifests/01-LVMO-Namespace.yaml
    sleep 2
    ...
    ready=false
    while [ "$timeout" -lt "1000" ]; do
        if [[ $(oc --kubeconfig=${EDGE_KUBECONFIG} get crd | grep lvmcluster.lvm.topolvm.io | wc -l) -eq 1 ]]; then
            ready=true
            break
        fi
    ...
    oc --kubeconfig=${EDGE_KUBECONFIG} apply -f manifests/04-LVMO-LVMCluster.yaml
    sleep 2
...
if ! ./verify.sh; then
    ...
    for edgecluster in ${ALLEDGECLUSTERS}; do
        ...
        if [ "${NUM_M}" -eq "3" ]; then
            install_odf_multinode $index $edgecluster
        else
            install_odf_sno $index $edgecluster
        fi
        index=$((index + 1))
    done
